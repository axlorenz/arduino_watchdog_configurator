...
public:
  Array();
  Array(const T &value);
  template <typename U, size_t N> Array(const U (&values)[N]);
  template <typename U, size_t N> Array(const Array<U, N> &values);
  ...
  T &back();
  void clear();
  template <typename U> void fill(const U &value);
  template <typename U, size_t N> void fill(const U (&values)[N]);
  template <typename U, size_t N> void fill(const Array<U, N> &values);
