...
template <size_t StatusSize = 1> class WdResponse {
  private:
    ...
    static constexpr uint8_t kInvalidCommandByte = 0x04; // Invalid command received
    ...
    static constexpr size_t kRawMsgSize = sizeof(kResponseStartByte1) +
                                          sizeof(kResponseStartByte2) + 1 + StatusSize +
                                          2; // Start bytes, Ack, Status bytes, CRC
  ...
  public:
    ...
    using StatusArray = std::array<uint8_t, StatusSize>; // Status bytes array
